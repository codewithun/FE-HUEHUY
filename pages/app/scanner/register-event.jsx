import { faArrowLeft, faCheckCircle, faPhone, faTicket, faUser, faWhatsapp } from '@fortawesome/free-solid-svg-icons';
...
export default function RegisterEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirect ke page booth dengan parameter registered=true
      setTimeout(() => {
        router.push(`/app/booth/${booth || 'BOOTH01'}?registered=true&customerId=${encodeURIComponent(formData.name)}&phone=${encodeURIComponent(formData.whatsapp)}`);
      }, 2000);
        ...
        <div className="container mx-auto min-h-screen bg-gradient-to-br from-cyan-50 flex items-center justify-center">
          <div className="text-center px-4 max-w-sm mx-auto">
            ...
            <p className="text-gray-600 mb-4 text-sm leading-relaxed">
              ...
              Anda akan dialihkan ke halaman booth.
            </p>
            <div className="flex items-center justify-center gap-2 mb-4 bg-white bg-opacity-40 backdrop-blur-sm rounded-full px-3 py-2 shadow-sm">
              ...
              <span className="text-xs text-gray-600">Mengalihkan ke booth event...</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-primary px-4 py-4 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-white/90">Event {booth || 'Curiosity'} - Registrasi</p>
            </div>
            <FontAwesomeIcon icon={faTicket} className="text-xl text-white/80" />
          </div>
        </div>
          ...
          <div className="bg-white bg-opacity-40 backdrop-blur-sm rounded-[25px] shadow-sm border border-gray-100 p-6">
            <div className="text-center mb-6">
              <div className="bg-primary/10 p-3 rounded-[15px] w-14 h-14 mx-auto mb-3 flex items-center justify-center">
                <FontAwesomeIcon icon={faTicket} className="text-xl text-primary" />
              </div>
              <h3 className="text-lg font-semibold text-gray-900 mb-1">Informasi Pendaftar</h3>
              <p className="text-sm text-gray-600">Isi data berikut untuk mengakses booth event</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-[15px] p-4 my-4">
                <div className="flex items-start gap-3">
                  <FontAwesomeIcon icon={faTicket} className="text-primary mt-0.5" />
                  <div>
                    <p className="text-sm font-medium text-primary mb-1">Setelah registrasi:</p>
                    <ul className="text-xs text-slate-600 space-y-1">
                      <li>• Akses langsung ke booth event</li>
